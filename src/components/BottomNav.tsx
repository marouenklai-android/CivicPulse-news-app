...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <nav className={`z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 py-1.5 px-2 transition-colors ${
      isAndroidFrame ? 'w-full rounded-b-[36px]' : 'fixed bottom-0 left-0 right-0'
    }`}>
      ...
      {isAndroidFrame && (
        <div className="pt-2 pb-1 flex justify-center">
          <div className="w-32 h-1 bg-slate-400/80 dark:bg-slate-600/80 rounded-full" />
        </div>
      )}
    </nav>
  );
